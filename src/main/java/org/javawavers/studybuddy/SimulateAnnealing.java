...
public class SimulateAnnealing {
    ...
    public static int[][] SchedulResult() {
        /*
    ...
    public static String printSchedule(int i, int j, int[][] schedule) {

        if (schedule == null) {
            // Return some error message or handle the null case
            return "κένο";
        }
        StringBuilder scheduleText = new StringBuilder();

            scheduleText.append("Μέρα: ").append(i).append("\n");

                if (schedule[j][i] > 0) {
                    scheduleText.append(besttask.get(i)).append("\n");
                } else {
                    scheduleText.append("Ελεύθερο").append("\n");
                }
